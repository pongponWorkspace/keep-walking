...
- **P1-X39 (`deploy-preview.yml` build tiles step):** clean checkout (CI, fresh clone) เคยล้มที่ `tools/tiles/bin/build.sh` ด้วย exit 1 เพราะ step "bbox vs 6 จังหวัด" อ่าน `tools/coverage/out/boundaries.geojson` ซึ่งเป็นผลของ coverage-survey pipeline เต็ม (ต้องโหลด OSM extract 327 MB) และถูก `.gitignore` (`out/`) ไม่ commit — มีเฉพาะเครื่องที่เคยรัน `tools/coverage/pipeline` มาก่อนเท่านั้น ส่วน exit 1 นี้เดิมถูก workflow เหมาเข้าข้อความ "no free host fits" (D-001) ทั้งที่ไม่เกี่ยวกับงบเลย แก้แล้วสองจุด: (1) `build.sh` ข้าม step นี้พร้อม log `[tiles] skip: ...` เมื่อไฟล์ไม่มี (กติกาเดียวกับที่ `tools/tiles/test/run.sh` ใช้อยู่แล้วสำหรับไฟล์เดียวกัน) เว้นแต่ `--region-mode provinces` ซึ่งต้องมีไฟล์นี้จริง จึง `die` ชัดเจน (2) `deploy-preview.yml` แยก exit 4 (`result=nohost`, ข้อความ D-001 เดิม) ออกจาก exit อื่นที่ไม่ใช่ 0/3/4 (`result=fail`, ข้อความใหม่ "generic error, see the Build tiles step log" ไม่มีคำว่า D-001) ยืนยันแล้วด้วย clean clone: `git clone` ใหม่ + `pnpm install --frozen-lockfile` + `bash tools/tiles/bin/build.sh` ได้ tile 13,502 ไฟล์ตรงกับ `tools/tiles/size-report.md` เป๊ะ, exit 0 · รายละเอียด: `infra/runbooks/preview-setup.md` หัวข้อ 9
- **P1-X40 (`deploy-preview.yml` env var ชนกับ `tools/tiles/bin/lib.sh`):** หลัง P1-X39 การรันจริงบน GitHub Actions ยังล้มที่ step "Build tiles" (`rc=1`) แล้วชน stop step ใหม่ของ P1-X39 (`result=fail`) log จริงคือ `[tiles] ERROR: config value .tools.pmtiles.version missing in infra/config/pages.json` สาเหตุ: `deploy-preview.yml` ประกาศ `env: TILES_CONFIG: infra/config/pages.json` ระดับ job (สำหรับ step "Read Pages project config" อย่างเดียว) แต่ `tools/tiles/bin/lib.sh` อ่านตัวแปรชื่อเดียวกัน `TILES_CONFIG` เป็นจุด override ของ `tools/tiles/config.json` เอง (มีเอกสารไว้ที่ `tools/tiles/README.md` และ `tools/tiles/size-report.md`) เพราะ `env:` ระดับ job มองเห็นได้ทุก step รวม "Build tiles" ด้วย จึงพา `build.sh` ไปอ่านไฟล์ config ของ Pages project แทนไฟล์ของตัวเอง แล้วพังที่คีย์แรกที่ไม่มี — **ไม่ใช่ปัญหาเฉพาะ Linux/runner** ทำซ้ำได้เป๊ะบน macOS ด้วยคำสั่งเดียว `TILES_CONFIG=infra/config/pages.json bash tools/tiles/bin/build.sh` แก้โดยเปลี่ยนชื่อตัวแปรใน `deploy-preview.yml` เป็น `PAGES_CONFIG` (ชื่อเดียวกับที่ `infra/scripts/lib.sh` ใช้อยู่แล้ว ไม่ชนกัน) พร้อมเพิ่ม step `actions/upload-artifact@v4` (`tile-build-logs`, `if: failure()`) อัปโหลด `tools/tiles/out/build.log` และไฟล์ text/JSON เล็กๆ อื่นของ build ให้มนุษย์ดาวน์โหลดได้เมื่อไม่มีสิทธิ์เข้า run log ยืนยันแล้วด้วย clean clone หลังแก้: `PAGES_CONFIG=infra/config/pages.json TILES_PUBLIC_BASE_URL=... bash tools/tiles/bin/build.sh` ได้ tile 13,502 ไฟล์ / deploy 14,283 ไฟล์ตรงตาม P1-X39, exit 0, `infra/scripts/publish-tiles.sh`/`publish-client.sh` ผ่านแบบ `DRY_RUN=1` ทั้งคู่, และ `pnpm lint && pnpm typecheck && pnpm test` เขียวบน host repo (927 ผ่าน, 2 skip) · รายละเอียด: `infra/runbooks/preview-setup.md` หัวข้อ 9
- **P1-X41 (`infra/pages/keep-walking-map/_headers` CORS ซ้อนกันบน tiles.json):** preview deploy รันสำเร็จ (run 36213296019) แต่แผนที่โหลดไม่ขึ้นบน `keep-walking-preview.pages.dev` — Chromium แจ้ง CORS error บน `.../tiles/<id>/tiles.json` ว่า `Access-Control-Allow-Origin` มีหลายค่า `curl -sI` ยืนยัน: `content-type: application/x-protobuf` (ควรเป็น `application/json`), `access-control-allow-origin: *, *`, `cache-control` ต่อกันสองชุด สาเหตุ: กฎ `/tiles/*` (rule ทั่วไปสำหรับ `.mvt`) และกฎ `/tiles/*/tiles.json` (rule เฉพาะ) แมตช์ path เดียวกัน เพราะ `*` เป็น splat ที่ข้าม `/` ได้ (ไฟล์เดียวกันพิสูจน์เรื่องนี้อยู่แล้ว: `/glyphs/*.pbf` ต้องข้าม `/` เข้าไปแมตช์ `glyphs/<font name>/<range>.pbf` ที่ซ้อน directory) Cloudflare Pages ใช้ทุก rule ที่ match และ**ต่อค่า header ชื่อเดียวกันด้วย ", "** แทนที่จะให้ rule เฉพาะเจาะจงกว่าชนะ (ยืนยันจาก curl evidence ตรงๆ ไม่ใช่แค่จากเอกสาร) แก้โดยให้กฎ `/tiles/*/tiles.json` ใช้ syntax `! Header-Name` (detach, https://developers.cloudflare.com/pages/configuration/headers/) ลบค่าที่ rule กว้างกว่าตั้งไว้ก่อน แล้วค่อยตั้งค่าของตัวเอง ครบทั้ง 5 header (`Content-Type`, `Cache-Control`, `Access-Control-Allow-Origin`, `Timing-Allow-Origin`, `X-Robots-Tag`) แม้บาง header ค่าที่ต้องการจะเหมือนกันทั้งสอง rule ก็ยังต้อง detach ก่อน เพราะ Cloudflare ต่อค่าซ้ำแม้ค่าจะเท่ากัน (`noindex, noindex`) เพิ่มเครื่องมือตรวจอัตโนมัติ `infra/scripts/lint-headers.py`/`.sh` (stdlib Python ล้วน ไม่ใช้เน็ต) ที่ parse `_headers` แล้วจำลองพฤติกรรม merge ของ Cloudflare กับ path ตัวอย่างคงที่ (tile, tiles.json, manifest.json, glyph pbf, font-face ttf, sprite json/png) — fail ทันทีถ้า path ไหนได้ header ชื่อเดียวกันจากสอง rule พิสูจน์แล้วว่าตรวจจับบั๊กเดิมได้จริงด้วย fixture สังเคราะห์ที่จงใจไม่ detach (`infra/scripts/test/fixture-headers/bad-overlap/`) และผ่านทั้งไฟล์จริงหลังแก้ (`infra/scripts/test/test-lint-headers.sh`, 9 case ผ่านหมด) **ยังไม่ได้ผูกเข้ากับ `pnpm test`/CI**: `vitest.config.ts` ไม่มี glob ที่ครอบคลุม `infra/` และไฟล์นั้นกับ `.github/workflows/*.yml` ไม่ได้อยู่ใน `writes` ของ task นี้ — ต้องมี follow-up task เพิ่ม `include` glob หนึ่งบรรทัดใน `vitest.config.ts` (รูปแบบเดียวกับ pytest-bridge ของ `tools/coverage`) หรือ step เรียก `infra/scripts/test/test-lint-headers.sh` ตรงใน `ci.yml` · รายละเอียด: `infra/runbooks/preview-setup.md` หัวข้อ 4 (curl check ใหม่) และหัวข้อ 9 (แถว troubleshooting ใหม่)
