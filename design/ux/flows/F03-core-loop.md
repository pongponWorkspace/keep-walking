...
Task: P1-F03-T16 · แก้ไขโดย P1-X17 (2026-09-23: F-03 นิยาม "นอกพื้นที่" ใช้ playarea mask, F-04 ทิศทาง A จอพกกระเป๋า/Wake Lock, F-05 แผนที่มาก่อนเลือกพลัง, F-07 ปุ่มยืนยันแยกใน age gate, F-09 แนะนำรอยแยกตามช่วงเลเวล) · เจ้าของ: uiux-designer · สถานะ: แก้ตาม `design/reviews/F03-design-gate-a.md` รอบ 1 รอ gate รอบ 2 · วันที่: 2026-09-23
...
### นาที 0–1: ปัญหา → consent → permission → อายุ → login → เห็นแผนที่ → เลือกพลัง (F-05)
...
4. `S-00-age-gate` — ตรวจอายุ 15+ ด้วยตัวเลือกช่วงอายุ/ปีเกิดจากรายการ (ไม่พิมพ์) `[age.gateTitle]` `[age.gateOptions]` เทียบกับ `config: privacy.minAge_yr` **แตะเลือกตัวเลือกยังไม่ไปต่อทันที ต้องกดปุ่มยืนยันแยก `[age.gateConfirm]` ("ยืนยัน") ก่อนเสมอ** (F-07, gate A หัวข้อ 4.8: การแตะพลาดบนรายการที่เลื่อนได้เกิดบ่อยบนมือถือ ผลของการเลือกผิดกระทบข้อมูลอายุที่มีผลทางกฎหมาย NN-7 ต้นทุนเพิ่มหนึ่งแตะรับได้ — ปุ่ม `age.gateConfirm` เป็น `.btn-disabled` จนกว่าจะเลือกตัวเลือกใดตัวเลือกหนึ่งก่อน เหมือนรูปแบบเดียวกับปุ่ม "เข้า" ใน B2/B3)
   ⤷ เงื่อนไข: ต่ำกว่าเกณฑ์ (หลังกดยืนยัน) → `[age.underMinTitle]` `[age.underMinBody]` เส้นทาง parental consent เป็น **scaffold ปิดไว้** ควบคุมด้วย `config: unlocks.parentalConsent.enabled` แสดงข้อความ "ยังเปิดใช้งานไม่ได้" เท่านั้น ไม่มีฟอร์มให้กรอกอะไร ไม่ปิดแอปทันที มีปุ่มเดียวกลับหน้าแรกของแอป (ไม่ไปต่อ flow onboarding ที่เหลือ)
   ⤷ เงื่อนไข: ผ่านเกณฑ์ (หลังกดยืนยัน) → `S-00-login`
5. `S-00-login` — เข้าสู่ระบบด้วย Google/Apple เท่านั้น (GDD "การเข้าสู่ระบบ") `[account.loginTitle]` ปุ่ม `[account.loginGoogle]` `[account.loginApple]` — ตำแหน่งของขั้นนี้ใน flow เป็น `mandatory-first` ตาม [ASSUMPTION A-P1-F03-T15-2 ของ ia.md] จนกว่า F07 จะตัดสิน guest session
   ...
   ⤷ เงื่อนไข: login สำเร็จ → `S-01-map` (F-05: แผนที่มาก่อนเลือกพลังเสมอ ดูขั้น 6–7)
6. `S-01-map` — เห็นแผนที่จริงครั้งแรกทันทีหลัง login พร้อมประเมินสถานะที่บ้านจริงจากตำแหน่ง ณ ขณะนั้น (ใกล้/ไกล/นอกพื้นที่/ไม่รู้ตำแหน่ง ตาม `ia.md` หัวข้อ 4) **ต้องมาก่อนขั้นเลือกพลังเสมอ** (F-05, gate A หัวข้อ 5.2: GDD นาที 0–1 สั่งชัดว่า "เห็นแผนที่และรอยแยกใกล้ตัว แล้วให้เลือกพลังทันที")
7. `S-00-class-select` — เปิดเป็น **drawer sheet ทับ `S-01-map`** ทันทีหลังขั้น 6 (ไม่ใช่หน้าเต็มจอแยก) เลือกพลัง 4 แบบ `[onboarding.pickRoleTitle]` การ์ด 4 ใบ (Tanker/Ranged/Support/Magic) แต่ละใบมีคำอธิบายผลสั้นบรรทัดเดียว ไม่มีสูตร `[onboarding.pickRoleTanker/Ranged/Support/Magic]` → กดเลือก 1 ใบ → ปิด sheet เข้าเกมทันที (ไม่มีหน้ายืนยันซ้อน) → กลับเห็นแผนที่เดิมที่สถานะไม่เปลี่ยน ปิดนาที 0–1
   ⤷ เงื่อนไข: สถานะที่ประเมินไว้ในขั้น 6 เป็น "ใกล้" → ไปนาที 1–3 ตามปกติ (ขั้น 8)
   ⤷ เงื่อนไข: สถานะเป็น "ไกล" (D1), "นอกพื้นที่" (D2) หรือ "ไม่รู้ตำแหน่ง" (D3) → sheet เลือกพลังยังขึ้นทับแผนที่ตามสถานะนั้นเหมือนกันทุกประการ (F-05 ยืนยันชัดกรณี D3) เลือกเสร็จเข้าสู่ **Flow D หัวข้อ 5** ทันที ข้ามนาที 1–3 ปกติ (ไม่มี "รอยแยกใกล้สุดในระยะ" ให้เดินตรง ๆ) — ไม่ต้องเริ่ม onboarding ใหม่ตั้งแต่ขั้น 1 (login/เลือกพลังทำไปแล้ว) พอผู้เล่นเดินเข้าใกล้ภายหลังจนสถานะกลับเป็น "ใกล้" ให้กลับเข้าขั้น 8 ต่อ

### นาที 1–3: เห็นรอยแยกใกล้สุด → เดินไปหา

8. บน `S-01-map` (สถานะ "ใกล้") ระบบแนะนำ **รอยแยกที่เปิดอยู่ใกล้สุดซึ่งช่วงเลเวลครอบเลเวลปัจจุบันของผู้เล่น ภายใน `config: unlocks.home.farDungeonThreshold_m (= 2000)`** ไม่ใช่แค่รอยแยกใกล้สุดเฉย ๆ (F-09, gate A หัวข้อ 5.2) เหตุผล: damage คูณ ×1.25 ต่อระดับที่ห่างจากช่วงเลเวลของโซน (GDD "damage มาจากไหน") ทำให้คนใหม่ที่เดินไปโซนเลเวลสูงกว่าตัวเองมากถูก auto-retreat เร็วผิดปกติจน "มาถึงแล้วเล่นไม่ได้" แผงล่างขึ้น `[map.nearestRiftFound]` ("พบรอยแยกใกล้คุณ — {distanceText} Lv.{levelMin}–{levelMax}") ปุ่มเด่นสุด `[map.navigateButton]` ("นำทาง") เปิดแอปนำทางภายนอกหรือ pin ทิศทางบนแผนที่เดิม (ตัดสินใจ implementation ภายหลัง ไม่ใช่ของ flow นี้)
   ⤷ เงื่อนไข: ไม่มีรอยแยกในระยะ `farDungeonThreshold_m` ที่ช่วงเลเวลครอบเลเวลผู้เล่น → แสดงรอยแยกที่เปิดอยู่ใกล้สุดตามเดิมพร้อมช่วงเลเวลจริง (ไม่บล็อกการเข้า ตาม GDD "ระดับเลเวลที่เหมาะสม" — ระยะ "ใกล้/ไกล" ของ pillars หัวข้อ 7 ยังคิดจาก dungeon ที่เปิดใกล้สุดทุกระดับเหมือนเดิม ไม่เปลี่ยนตามเงื่อนไขนี้)
   ⤷ เงื่อนไข: ระยะรอยแยกที่แนะนำ (หรือรอยแยกใกล้สุดถ้าไม่มีตัวที่ตรงเลเวล) เกิน `config: unlocks.home.farDungeonThreshold_m (= 2000)` → ไม่ใช่ onboarding ปกติอีกต่อไป ข้ามไป Flow D (หัวข้อ 5) ตั้งแต่ตอนนี้ — เกมยังคงสถานะ "ยังไม่จบ onboarding" ไว้ พอผู้เล่นเดินเข้าใกล้ในภายหลังแล้วกลับมาอยู่ในระยะ ให้กลับเข้า flow A ขั้นที่ 9 ต่อ
   ⤷ เงื่อนไข: อยู่ในระยะ → ผู้เล่นเดินไปจริง (ไม่มีอะไรให้ทำในแอประหว่างเดิน มือถือเก็บกระเป๋าได้ ดูหัวข้อ 4.9 เรื่องจอพกกระเป๋า — ใช้เฉพาะระหว่างอยู่ใน run เท่านั้น ไม่ใช่ช่วงเดินไปหา dungeon)
...
### 4.9 จอพกกระเป๋า (Wake Lock) — ทิศทาง A (F-04)

ที่มา: game-director ตัดสินทิศทาง A ใน `design/reviews/F03-design-gate-a.md` หัวข้อ 4.4 แทนทางเลือก B เดิม เพื่อให้ "มือถืออยู่ในกระเป๋าได้" (GDD Core loop ใน dungeon) กับ "ล็อกหน้าจอ (v1 เว็บ) หยุดนับ movement" (GDD สัญญาณขาดและแอปถูกปิด) ไม่ขัดกันในทางปฏิบัติ — สเปก component เต็มอยู่ที่ `components.md` หัวข้อ 12 ตัวอย่างจริงที่ `wireframes/03-run-active.html` เฟรม C0/C0b

1. **เข้าจอพกกระเป๋าอัตโนมัติ:** ทันทีที่ server ยืนยัน B5 (เข้า run สำเร็จ) client เรียก `navigator.wakeLock.request('screen')` และสลับ `S-03-run` เป็น **จอพกกระเป๋า** เป็นค่าเริ่มต้น: พื้นมืดเกือบเต็มจอ มีแค่ตัวเลข HP ใหญ่ `[run.pocketHp]` เวลาถึง tick ถัดไป `[run.pocketNextTick]` และป้ายเล็กยืนยันว่ายังนับระยะอยู่ `[run.pocketCounting]` contrast สูง ไม่มี animation ต่อเนื่อง (P4)
2. **กันแตะพลาด:** ไม่มีปุ่มที่มีผลต่อ run บนจอพกกระเป๋า (ไม่มีปุ่มออกเอง ไม่มี quick command) ออกจากจอพกกระเป๋าด้วย gesture ที่ตั้งใจ — **ปัดขึ้นค้าง** (`swipe-up-hold`, ระยะเวลาขั้นต่ำเป็นรายละเอียด implementation ของ gameplay-programmer แต่ต้องนานพอกันปัดผ่านโดยบังเอิญ) แสดงคำใบ้ `[run.pocketWakeHint]` ครั้งแรกที่เข้าจอนี้ของบัญชีเท่านั้น (ไม่ใช่ tutorial ซ้ำ) — ปัดสำเร็จแล้วกลับสู่จอ run ปกติ (4.1–4.7 เดิมทั้งหมด) พร้อมปุ่ม `[run.pocketEnterButton]` ("กลับไปจอพกกระเป๋า") ให้กดกลับเข้าจอพกกระเป๋าเองเมื่อพร้อมเก็บมือถือต่อ (ไม่กลับอัตโนมัติตามเวลา กันจอมืดกะทันหันระหว่างกำลังใช้ยา/ดู nearby party อยู่)
3. **เสียง/สั่นยังทำงาน:** เพราะหน้าเว็บยัง foreground cue ของ HP ต่ำ (4.2), auto-retreat (4.4), ตาย (4.5), tick (4.6) เล่นได้ปกติแม้จอมืดเกือบเต็ม (ช่วยชดเชย iOS ที่ไม่มี `navigator.vibrate()`, ดูความเสี่ยง 4.6 ของ gate A)
4. **ปล่อย Wake Lock เสมอเมื่อจบ run ทุกทาง:** สรุปผล (4.8), auto-retreat (4.4), ตายแล้วออก, ปิดฉุกเฉิน (Ended ใน 4.1) — ไม่ขอ Wake Lock ที่หน้าอื่นนอก `S-03-run` · ถ้า tab ถูกซ่อน (visibility hidden) OS อาจปล่อย wake lock เองอัตโนมัติ client ต้อง request ใหม่ทันทีที่ visibility กลับมา visible โดยไม่ต้องแจ้งผู้เล่น
5. **ทางสำรอง B (Wake Lock ใช้ไม่ได้):** ถ้า browser ไม่รองรับ `navigator.wakeLock` หรือ request ล้มเหลว ไม่แสดงจอพกกระเป๋า ใช้จอ run ปกติแทน พร้อม toast `[run.screenLockNotice]` ("จอดับ ระยะก็หยุดนับ เปิดจอไว้ระหว่างเดิน") **ครั้งเดียวต่อบัญชี** ตอนเข้า run ครั้งแรก (เดิม A-P1-F03-T16-2) — ถ้าผู้เล่นล็อกจอเองอยู่ดี เมื่อกลับมาแสดง `[gps.suspended]` ("ช่วงจอดับไม่ได้นับระยะ") ตามข้อเท็จจริง ไม่ใช่ tutorial ซ้ำ
6. **ช่วงหน้าเว็บ hidden ไม่นับระยะ ไม่ย้อนเติม ไม่ประมาณ** (เหมือนเดิมในหัวข้อ 8) tick ที่ไม่ผ่านเพราะช่วง hidden แสดงผลเหมือน `[run.tickDenied]` ปกติ ไม่ลงโทษ
7. **ปิดจอพกกระเป๋าได้ในตั้งค่า:** หน้าย่อย "การเดินและความปลอดภัย" (Flow E) มี toggle `[settings.pocketScreenLabel]` ("จอพกกระเป๋า") ค่าเริ่มต้น = เปิด **ไม่ต้องยืนยันตอนปิด** (ต่างจาก auto-retreat) เพราะไม่ใช่ระบบกันตาย แค่ทางเลือกการแสดงผล — ปิดแล้วจอ run กลับไปแสดงผลปกติเต็มข้อมูลตลอด (ไม่มีจอพกกระเป๋าให้ปัดออก/กลับเข้าอีก) แต่ยัง request Wake Lock เหมือนเดิมเพื่อกัน movement หยุดนับ
8. **เกณฑ์ถอยกลับไป B ทั้งระบบ:** ถ้า Phase 2 (tech-lead + product-manager, F-17) วัดแบตต่อชั่วโมงในเครื่องทดสอบทั้ง Android และ iOS เกินงบที่ตกลงไว้ ให้ส่งกลับ game-director ตัดสินใหม่ ไม่ถอยเงียบ (gate 4.4 ข้อ 8)

## 5. Flow D — dungeon ไกล / นอกพื้นที่ / ไม่รู้ตำแหน่ง

ที่มา: pillars หัวข้อ 7 (สถานะที่บ้าน) และ ia.md หัวข้อ 4 เอกสารนี้เพิ่มลำดับขั้นเป็น flow ให้ชัดว่าเข้า-ออกจากสถานะเหล่านี้ตอนไหน `S-01-map` เป็นหน้าเดียวเสมอ เปลี่ยนแค่เนื้อหาแผงล่าง ประเมินสถานะใหม่ทุกครั้งที่เปิดแอปและทุกครั้งที่ตำแหน่งเปลี่ยนเกิน `config: unlocks.home.reevaluateDistance_m` — คนใหม่เข้าสถานะเหล่านี้ได้โดยตรงจาก Flow A ขั้น 6–7 (F-05) ไม่ต้องผ่านนาที 1–3 ปกติก่อน

**นิยาม "นอกพื้นที่" (F-03, D-064, gate A หัวข้อ 4.5):** เงื่อนไขเดียวคือตำแหน่งอยู่นอกรูของ `config: unlocks.home.seeOutOfAreaMask` (ชี้ไปที่ `data/map/playarea-mask.geojson` — geometry เดียวกับที่ server ใช้ใน Phase 3, D-043) **ไม่ใช่ระยะทางถึง dungeon ที่เปิดอยู่** ไม่ว่าจะไกลแค่ไหนก็ตาม ตราบใดที่ตำแหน่งยังอยู่ในรูของ mask ให้ถือเป็น D1 (ไกล) เสมอ ไม่ใช่ D2 — คีย์เดิม `unlocks.home.outOfServiceAreaThreshold_m` (20,000 ม.) ถูกถอดออกแล้วใน P1-X18 และต้องไม่กลับมาใช้อีก

D1. ผู้เล่นอยู่**ในรูของ mask** (`unlocks.home.seeOutOfAreaMask`) แต่ dungeon ที่เปิดอยู่ใกล้สุดไกลกว่า `config: unlocks.home.farDungeonThreshold_m (= 2000 ม.)` → แผง `S-06-far-dungeon-panel` ทับแผนที่: ระยะจริง `[home.farBody]` ({distanceText}) ไม่หลอกว่าใกล้ ปุ่มเด่นสุด `[home.farNavigate]` ("นำทาง") ยังพาไปเดินจริงแม้ไกล ทางลัด `[home.farRoleInfoLink]` → `S-05-role-info` และ `[home.farProfileLink]` → `S-10-profile`
...
D2. ตำแหน่งอยู่**นอกรูของ** `config: unlocks.home.seeOutOfAreaMask` — เงื่อนไขเดียว ไม่มีตัวเลขระยะทางเกี่ยวข้อง ไม่มี "หรือ back office กำหนด" แยกต่างหากอีก (ค่าเดียวกับที่ server ใช้ตัดสินโซนเล่นได้ใน Phase 3) → แผนที่ส่วนที่อยู่นอกรูของ mask เป็นโซนดำมีเส้นแบ่งจังหวัด นำทางไม่ได้ แผง `S-07-out-of-area-panel`: กรอบ `[home.outOfAreaTitle]` ("ช่วยกันปลุกจังหวัดเรา") จำนวนคนลงทะเบียนต่อจังหวัด `[home.outOfAreaCount]` ปุ่มเด่นสุด `[home.outOfAreaCta]` ("ลงทะเบียนความสนใจ") → `S-09-interest-register` ทางลัดไป role-info และ profile เหมือน D1 — การตรวจว่าอยู่ในรูของ mask ทำที่ client ได้เพราะเป็นการแสดงผลเท่านั้น ไม่มีผลต่อรางวัล (NN-1 ไม่กระทบ)
...
- **ตัวเลือก `[settings.pocketScreenLabel]` ("จอพกกระเป๋า", F-04, gate A 4.4 ข้อ 7)** อยู่หน้าย่อยเดียวกัน พร้อมคำอธิบาย `[settings.pocketScreenHint]` ค่าเริ่มต้นเปิด **ไม่ต้อง popup ยืนยันทั้งเปิดและปิด** (ไม่ใช่ระบบกันตาย ต่างจาก auto-retreat ด้านบน — ดู Flow C หัวข้อ 4.9 ข้อ 7) ปิดแล้วจอ `S-03-run` แสดงผลปกติเต็มข้อมูลตลอด ไม่มีจอมืดให้ปัดออกอีก แต่ยัง request Wake Lock เหมือนเดิมเพื่อกัน movement หยุดนับ
...
- **Flow C (run) — จุดที่ต้องระวังที่สุด:**
  ...
  - **ล็อกหน้าจอ / จอพกกระเป๋า (v1 เว็บ, แก้ตาม F-04 gate A 4.4):** ทิศทาง A (ดูหัวข้อ 4.9) — client ขอ Wake Lock ทันทีที่เข้า run และสลับเป็นจอพกกระเป๋าเป็นค่าเริ่มต้น เพื่อกันจอดับ/ล็อกจริงระหว่างเดิน (นับ movement ต่อได้เพราะหน้าเว็บยัง foreground จริง) ถ้า Wake Lock ใช้ไม่ได้ (browser ไม่รองรับ/request ล้มเหลว) หรือผู้เล่นกดล็อกจอเองอยู่ดี browser จะไม่ให้ background location ตามข้อจำกัดจริงของ v1 บนเว็บ → **หยุดนับ movement ทันทีที่ล็อกจอจริง** (ไม่ใช่ bug) กลับมาเจอ `[gps.suspended]` ตามข้อเท็จจริง ไม่ใช่ tutorial ซ้ำ — เมื่อขึ้น native (M8) ข้อจำกัดนี้หมดไป เอกสารนี้บันทึกไว้เป็น `[ASSUMPTION A-P1-F03-T16-2]` (ปรับปรุงตาม F-04)
...
| `age.gateBody` | message | คำอธิบายสั้นก่อนตัวเลือกปีเกิด | (เกมนี้สำหรับอายุ {minAge} ปีขึ้นไป เลือกปีเกิดจากรายการ) |
| `age.gateOptions` | label | ป้ายของรายการปีเกิด (เลือกจากรายการ ไม่พิมพ์) | (เลือกปีเกิด) |
| `age.gateConfirm` | button | **ปุ่มยืนยันแยก (F-07, gate A 4.8)** — `.btn-disabled` จนกว่าจะเลือกตัวเลือกใดตัวเลือกหนึ่งก่อน แตะตัวเลือกอย่างเดียวยังไปต่อไม่ได้ | (ยืนยัน) |
...
| `run.pocketHp` | label | ตัวเลข HP ใหญ่บนจอพกกระเป๋า (F-04, หัวข้อ 4.9) | (HP {hpPct}%) |
| `run.pocketNextTick` | label | เวลาถึง tick ถัดไปบนจอพกกระเป๋า | (อีก {timeLeft} ได้ของ) |
| `run.pocketCounting` | label | ป้ายเล็กยืนยันว่ายังนับระยะอยู่ | (กำลังนับระยะให้อยู่) |
| `run.pocketWakeHint` | message | คำใบ้ gesture ออกจากจอพกกระเป๋า ครั้งแรกของบัญชีเท่านั้น | (ปัดขึ้นค้างเพื่อดูปุ่ม) |
| `run.pocketEnterButton` | button | ปุ่มกลับเข้าจอพกกระเป๋าเอง หลังปัดออกมาดูจอปกติแล้ว | (กลับไปจอพกกระเป๋า) |
| `run.screenLockNotice` | message | ทางสำรอง B เมื่อ Wake Lock ใช้ไม่ได้ ครั้งเดียวต่อบัญชี (หัวข้อ 4.9 ข้อ 5) | (จอดับ ระยะก็หยุดนับ เปิดจอไว้ระหว่างเดิน) |

### 9.3a `nav.shop`, `shop.*`, `market.*` (ร้าน NPC หลังจบ run แรก, F-01)

ไม่อยู่ในนาที 0–10 (ปลดเมื่อจบ run แรกที่มี tick ผ่าน gate เท่านั้น) รายชื่อ key ทั้งหมดจาก narrative-designer (P1-X20) — ผูกกับหน้าที่ใน `ia.md` หัวข้อ 3.4

| key | kind | บริบท | ร่างไทย |
| --- | --- | --- | --- |
| `nav.shop` | label | แท็บล่างที่ 2 ("ร้าน") ปลดพร้อม `unlocks.npcShop` | (ร้าน) |
| `shop.title` | label | `S-18a-npc-shop` หัวจอ — ชื่อผู้เล่นเห็น "ร้านปากซอย" | (ร้านปากซอย) |
| `shop.subtitle` | label | คำโปรยใต้หัวจอ (ถ้อยคำจริงเป็นของ narrative-designer) | (ของกินของใช้ราคาคงที่) |
| `shop.sellTab` | label | แท็บย่อยขายวัตถุดิบ | (ขาย) |
| `shop.buyTab` | label | แท็บย่อยซื้อยา | (ซื้อยา) |
| `shop.sellToShopButton` | button | ปุ่มขาย — ใช้ทั้งใน `S-18a-npc-shop` และเป็นปุ่มลัดใน `S-11-inventory` | (ขายให้ร้าน) |
| `shop.sellPrice` | label | ราคาต่อชิ้นตอนขาย (data จาก `economy.json#npcPricing`) | ({price} ทอง/ชิ้น) |
| `shop.sold` | message | toast หลังขายสำเร็จ | (ขายแล้ว) |
| `shop.sellEmpty` | label | ไม่มีของให้ขาย | (ไม่มีของให้ขายตอนนี้) |
| `shop.equipmentNotSellable` | label | อุปกรณ์ที่สวมใส่อยู่ขายไม่ได้ | (อุปกรณ์นี้ขายไม่ได้) |
| `shop.buyPotionButton` | button | ปุ่มซื้อยา | (ซื้อ) |
| `shop.potionPrice` | label | ราคายา (data จาก `economy.json#potions`) | ({price} ทอง) |
| `shop.potionHeal` | label | ผลของยาฟื้น HP | (ฟื้น HP {healPct}%) |
| `shop.potionRevive` | label | ผลของยาฟื้นจากตาย | (ฟื้นจากตายทันที) |
| `shop.bought` | message | toast หลังซื้อสำเร็จ | (ซื้อแล้ว) |
| `shop.notEnoughGold` | message | เงินไม่พอซื้อ | (เงินไม่พอ) |
| `shop.unlocked` | message | toast ครั้งเดียวตอนปลด `unlocks.npcShop` | (ร้านปากซอยเปิดแล้ว) |
| `market.title` | label | `S-18-market` หัวจอ | (ตลาด) |
| `market.listButton` | button | ปุ่มลงขายในตลาด ใน `S-11-inventory` | (ลงขายในตลาด) |
...
| `settings.pocketScreenLabel` | label | toggle จอพกกระเป๋า ในหน้าย่อยเดียวกัน (F-04, Flow E) — เปิด/ปิดไม่ต้อง popup ยืนยัน | (จอพกกระเป๋า) |
| `settings.pocketScreenHint` | message | คำอธิบายใต้ toggle | (ปิดจอเป็นจอมืดมี HP ระหว่างเดิน กันแตะพลาดในกระเป๋า) |
...
### สมมติฐาน
- A-P1-F03-T16-1 (แก้ลำดับตาม F-05): การใส่ consent location + permission เบราว์เซอร์ + อายุ 15+ + login เข้าไปในนาที 0–1 (ตามที่ board กำหนด) ทำให้ขั้นตอนก่อนเห็นแผนที่มี 5 จอย่อย (intro, consent, priming, age + ปุ่มยืนยันแยก, login) ตามด้วยแผนที่จริงแล้วค่อยขึ้น sheet เลือกพลังทับแผนที่ แม้แต่ละจอสั้นมาก ความเสี่ยงคือรวมกันอาจเกินความรู้สึก "นาทีเดียว" ของผู้เล่นจริงโดยเฉพาะถ้า native permission prompt ของ OS ช้าหรือผู้เล่นลังเลตอน login — เอกสารนี้ออกแบบให้แต่ละจอมีจุดโฟกัสเดียวและปุ่มเดียวเพื่อลดเวลาให้สุด แต่ตัวเลขเวลาจริงต้องวัดจาก field walk (P1-F02-T20/T14) ไม่ใช่คาดเดา (ยืนยัน: game-director, product-manager ในการอ่านผล field walk)
- A-P1-F03-T16-2 (แก้ตาม F-04, ปิดบางส่วน): v1 บนเว็บหยุดนับ movement ทันทีที่ล็อกหน้าจอจริง (ตาม GDD) — **game-director ตัดสินทิศทาง A** (Wake Lock + จอพกกระเป๋า, `design/reviews/F03-design-gate-a.md` หัวข้อ 4.4) เป็นค่าเริ่มต้น แทน "คำเตือนครั้งเดียว" (ทางเลือก B เดิม) ซึ่งยังอยู่เป็นทางสำรองเมื่อ Wake Lock ใช้ไม่ได้ (แสดง `run.screenLockNotice` ครั้งเดียวต่อบัญชี) รายละเอียดเต็มอยู่ Flow C หัวข้อ 4.9 — **[ASSUMPTION A-P1-X17-1] gesture ออกจากจอพกกระเป๋าคือ "ปัดขึ้นค้าง" (swipe-up-hold) ยืนยันตามที่ narrative-designer เสนอใน P1-X20 (ยืนยัน: gameplay-programmer กำหนดระยะเวลาขั้นต่ำจริง, tech-lead ยืนยันความเป็นไปได้ของ Wake Lock ใน F-17)**
...
- [ASSUMPTION A-P1-X17-2]: ร้าน NPC (`S-18a-npc-shop`) ได้แท็บล่างของตัวเอง (`nav.shop`) ทันทีที่ปลด `unlocks.npcShop` แทนที่จะเป็นแค่ปุ่มใน `S-11-inventory` เพราะใช้บ่อยกว่า enhance/market มาก (คำตัดสิน uiux ปิด Q-T15-1 บางส่วน ดู `ia.md` หัวข้อ 2/3.4/6) — ระบบอื่นที่เหลือ (ตีบวก, ตลาด) ยังไม่ได้แท็บของตัวเอง (ยืนยัน: game-director, art-director ผลต่อ layout bottom bar 2 แท็บ)
...
รายการ copy key ทั้งหมดในหัวข้อ 9 (รวม 9.3a `nav.shop`/`shop.*`/`market.*` ใหม่จาก F-01 และ `run.pocket*`/`settings.pocketScreen*` ใหม่จาก F-04) ตรงกับที่ narrative-designer สร้างจริงใน P1-X20 แล้ว ไม่มี key ใดในเอกสารนี้ต่างจากที่ narrative ประกาศ · ชื่อ key กลุ่ม `gps.*` (หัวข้อ 9.5) ประกาศให้ gameplay-programmer (P1-F02-T10) ใช้ตรงกัน · คีย์ config ที่เอกสารนี้เคยรอ (`unlocks.home.reevaluateDistance_m`, `privacy.positionLogTtl_s`, `privacy.minAge_yr`, `unlocks.npcShop`, `unlocks.home.seeOutOfAreaMask` และอีก 2 จุดที่เปลี่ยนชื่อ ดู A-P1-F03-T16-5) มีค่าจริงครบแล้วจาก P1-H03/P1-X18 (`config/balance/unlocks.json`, `location.json`, `privacy.json`) — ชื่อ key ในเอกสารนี้แก้ตามของจริงแล้ว
