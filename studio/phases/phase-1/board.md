...
| P1-F02-T17 | F02 | สมัคร Cloudflare Free (ไม่ผูกบัตร) + สร้าง Pages project + API token สิทธิ์จำกัด เก็บใน GitHub Actions secret / `.env.local` (D-008) | research | HUMAN | — | — (ถ้ามี token เก็บใน `.env.local` หรือ GitHub Actions secret เท่านั้น ไม่ commit) | DONE | 2026-09-25 บัญชี Cloudflare พร้อม · ได้ Worker keep-walking.pongponcrb.workers.dev (ไม่ใช้) แทน Pages project · publish-client.sh สร้าง keep-walking-preview เอง · รอยืนยัน secrets/2FA/ไม่มีบัตร |
| P1-F02-T18 | F02 | สร้าง GitHub repo **public**, เปิด secret scanning + push protection, push ครั้งแรก, ยืนยัน CI รัน | research | HUMAN | P1-F02-T07 | — | DONE | push main 2026-09-25 · CI run 36155723116 เขียวทั้ง 4 job · Secret scanning/Push protection: รอ user ยืนยัน |
...
| P1-X39 | F02 | deploy-preview run 36157061484: tools/tiles/bin/build.sh exit 1 บน ubuntu-latest (ไม่ใช่ผลงบ exit 3/4) · หา root cause จาก clean clone แก้ให้ build ได้ และแยกข้อความ rc=4 ออกจาก error ทั่วไป | fix | devops-engineer | P1-F02-T17 | `.github/workflows/deploy-preview.yml`, `tools/tiles/bin/`, `infra/scripts/` | IN_PROGRESS | — |
