...
8. **ห้ามแตะ movement gate หรือระบบกันตายผ่าน event ใดๆ** — `config/balance/dungeons.json#movementGate.*` (`minDistancePerWindow_m`, `window_s`, `comparison`, `appliesTo`, `exceptions`) และ `#hpSafety.*` (`autoRetreatEnabledByDefault`, `autoRetreatThreshold_pct`, `autoRetreatKeepsRunLoot`, `lowHpWarningThreshold_pct`, `lowHpWarningChannels`) ไม่อยู่ในขอบเขตของปฏิทินหรือ tuning playbook นี้เลย ไม่มีปฏิทินหรือ event แถวใดผ่อน เร่ง หรือยกเว้น gate ทั้งสองชุดนี้ไม่ว่าจะอ้างเหตุผลใด (เช่น "ฤดูฝนลด gate ครึ่งหนึ่ง") รายละเอียดและเหตุผลอยู่ที่ `ops/tuning-playbook.md` หัวข้อ 1.1 (NN-8 ข้อ 3, pillars 7.3 ข้อ 1)
9. **ประกาศ raid ส่งเฉพาะผู้เล่นที่ปลดแล้ว** — หน้าประกาศ raid ในแอปและ push ทุกชนิด (รวม event พิเศษที่แตะ raid) ส่งเฉพาะผู้เล่นที่ผ่าน `config: unlocks.raid` (L5 + 3 run ที่นับ ตาม D-040) เท่านั้น ผู้เล่นที่ยังไม่ปลดยังเข้าร่วมทางกายภาพได้ตามปกติถ้ายืนอยู่ในวงบอสระหว่าง raid (popup เลือก dungeon/boss ตัวเดียวกัน) แต่ต้องไม่เห็นป้ายล่วงหน้าบนแผนที่และไม่ได้รับ push ของ raid ก่อนปลด (pillars 6.2 U3, gate A การตัดสิน 4.2)
...
| C. ฤดูฝน | มาตรการรับมือ DAU ตกช่วงฝน (GDD "ความเสี่ยงที่ต้องเฝ้าดู") **จำกัดเฉพาะการลดต้นทุนของสิ่งที่ผู้เล่นได้จากการเดินมาแล้ว** เช่น ลดต้นทุนตีบวกชั่วคราว (วัตถุดิบ/gold ต่อครั้งใน `enhance.json`) — **ห้ามทุกกรณี:** สร้าง gold, exp, drop หรือวัตถุดิบใหม่ที่บ้าน, เพิ่มราคาขาย NPC, ลดภาษีตลาดผู้เล่น, หรือผ่อน movement gate (F-06, pillars 7.3 ข้อ 1 และ 6, หัวข้อ 1 ข้อ 8) | เปิดเมื่อ GR-10 (DAU ฝนเทียบวันแห้งตกเกิน 30%) หรือตามฤดูกาลที่คาดไว้ล่วงหน้า (พ.ค.–ต.ค.) | liveops-operator / systems-designer (ค่าที่เปลี่ยนต้องอยู่ในขอบเขต tuning playbook หัวข้อ 3 และเป็นการลดต้นทุนของที่มีอยู่แล้วเท่านั้น) |
...
| --- | --- | --- | --- | --- | --- | --- |
| W1–W4 | Raid ทุกสัปดาห์ (α เปิดตัว) | A | `raid.json#bossHp.alphaLaunch` = 0.45 ตามค่าตั้งต้น 30 วันแรก | ประกาศ raid ก่อนวัน (ดู `ops/raid-playbook.md` เมื่อเขียนแล้ว) — ส่งเฉพาะผู้เล่นที่ผ่าน `config: unlocks.raid` (F-14, หัวข้อ 1 ข้อ 9) | liveops-operator / game-director | ไม่มีการเปลี่ยนค่าในช่วงนี้ (ใช้ค่า config ตั้งต้นตาม GDD) จึงไม่มี rollback ที่ต้องทำ |
...
| W9–W12 | ช่วงต้นฤดูฝน (ตัวอย่าง) | C | ลดต้นทุนตีบวกชั่วคราวเท่านั้น (`enhance.json` เช่นวัตถุดิบ/gold ต่อครั้ง) ภายใน ±20% ของค่าตั้งต้น — **ห้าม**แก้ `economy.json#npcSellPrice_gold` ให้สูงขึ้น, **ห้าม**แก้ `economy.json#marketTax` ให้ต่ำลง, **ห้าม**เพิ่ม gold/exp/drop/วัตถุดิบใหม่ใดๆ ที่บ้าน และ**ห้าม**แตะ `dungeons.json#movementGate.*`/`#hpSafety.*` ไม่ว่ากรณีใด (F-06, หัวข้อ 1 ข้อ 8) | narrative-designer เตรียม copy "ฝนตก น้ำขังชั่วโมง กลับมาตีบวกที่บ้านก่อน" (ของที่ลดต้นทุนคือของที่มีอยู่แล้วจากการเดิน ไม่ใช่ของใหม่) | liveops-operator / systems-designer (ค่าที่แก้), game-director (ถ้าเกิน ±20%) | เปิดเฉพาะช่วงเวลาที่ตั้งไว้ล่วงหน้า (start/end ชัดเจน) คืนค่าปกติอัตโนมัติเมื่อครบกำหนด · ถ้า GR-10 ยังตกต่อเนื่องหลังปรับ ส่ง handoff ถึง product-manager ทบทวนมาตรการ (ห้ามแก้ด้วยการเพิ่มรางวัลที่บ้านหรือผ่อน gate) |
...
| DAU ฤดูฝนตก | GR-10: DAU ช่วงฝนตกไม่เกิน 30% เทียบวันแห้ง | เปิด event ประเภท C (หัวข้อ 2, 3) ล่วงหน้าก่อนฤดูฝน (พ.ค.) ไม่ใช่ตอบสนองทีหลัง · ขอบเขตของมาตรการคือหัวข้อ 1 ข้อ 8 เท่านั้น (ลดต้นทุนของที่มีอยู่แล้ว ห้ามสร้างรางวัลใหม่ที่บ้าน ห้ามผ่อน gate) · ถ้า GR-10 ยังตกเกินหลังมาตรการ ส่ง handoff ถึง product-manager และ systems-designer ทบทวนมาตรการที่บ้านเพิ่ม (ยังต้องอยู่ในกรอบห้ามสร้างรางวัลเดียวกัน) | systems-designer (ค่าที่แก้) / game-director (เกิน ±20%) |
...
- [ASSUMPTION A-P1-F03-T20-3]: ตัวคูณ exp/drop ชั่วคราวของ event **ประเภท B เท่านั้น** (เทศกาลไทย เกิดขึ้นระหว่างเดินเข้า dungeon จริง ผ่าน movement gate ปกติ) ต้องอยู่ในขอบเขต ±20% ของ `ops/tuning-playbook.md` เสมอ ถ้าต้องการมากกว่านั้นต้องเป็น decision ของ game-director แยกต่างหาก ไม่ใช่ event ปกติ · **ประเภท C (ฤดูฝน) ไม่มีตัวคูณ exp/drop หรือรางวัลใหม่ใดๆ ที่บ้านเลย** (F-06) มีได้เฉพาะการลดต้นทุนของสิ่งที่ผู้เล่นได้จากการเดินมาแล้วตามหัวข้อ 1 ข้อ 8 และหัวข้อ 2 ประเภท C
